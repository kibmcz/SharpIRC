...
namespace FuturamaWiki
{
    public class FuturamaWiki : PluginInterface, Plugin
    {
        ...
        {
            return "Futurama Wikipedia Search";
        }
        ...
        {
            if (string.Equals(Functions.getcommand(message), "futurama", StringComparison.OrdinalIgnoreCase))
            {
                var cmessage = message.Substring(Functions.getcommand(message).Length + 2);
                cmessage = HttpUtility.UrlEncode(cmessage);
                var address = "http://futurama.wikia.com/wiki/Special:Search?search=" + cmessage + "&go=1&x=0&y=0";
                ...
                foreach (string line in lines)
                {
                    ...
                    if (line.Contains("var wgTitle") && WSDesc.Length > 0)
                    {
                        ...
                        Commands.Send_PrivMsg(connection, channel, IFCommands.bold + "Futurama Wikipedia " + IFCommands.bold + WSTitle + " - http://futurama.wikia.com" + WSLink);
                    ...
                    {
                        Commands.Send_PrivMsg(connection, channel, IFCommands.bold + "Futurama Wikipedia " + IFCommands.bold + "OOooo a reception table with muffins! Oh wait it was nothing :( Sorry dude your search returned nothing.");
                    }
                }
                WSDesc = "";
                WSLink = "";
