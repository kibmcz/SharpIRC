    ...
    public static class SpecialExtentions {
        ...
        public static bool IsCommand(this string msg, string cmd) {
            if (msg.StartsWith(Program.GlobalSettings.CommandPrefix.ToString())) {
                return (String.Equals(msg.Split(' ')[0].Substring(1), cmd, StringComparison.OrdinalIgnoreCase)) ? true : false;
            }
            Match myMatch = Regex.Match(msg, @"\(.*?\) " + Program.GlobalSettings.CommandPrefix + @"(.*)");
            if (myMatch.Success) {
                return (String.Equals(myMatch.Groups[1].Value.Split(' ')[0], cmd, StringComparison.OrdinalIgnoreCase)) ? true : false;
            }
            return false;
        }
        ...
        public static IRCUser IRCUserFromString(this string nick, IRCConnection connection) { 
            ...
            foreach (IRCUser user in from list in connection.Nicklist from user in list.Nicks where user.Nick == nick select user) {
                getUser = user;
            }
            return getUser;
        }
        ...
        public static bool IsIgnored(this string host) {
            ...
            bool exists = false;
            foreach (IgnoredUser user in Program.IgnoreList.Ignores.Where(user => user.Host == host)) exists = true;
            return exists;
        }
