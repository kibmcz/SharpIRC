    ...
    public class System : PluginInterface {
        public override void ChanMsg(ChannelMessage message) {
            ...
            if (message.Message.IsCommand("version") && message.Sender.hasCommandPermission(message.Connection, message.Channel, "version")) {
                Commands.SendPrivMsg(message.Connection, message.Channel, String.Format("SharpIRC IRC Bot v.{0} -  Changeset: http://sharpirc.codeplex.com/SourceControl/changeset/{1} - Branch: {2}  -  http://sharpirc.codeplex.com/", Program.Version, Program.Revision.RevisionID, Program.Revision.Branch));
            }
