...
namespace SharpIRC {
    [Extension] public class Clone : PluginInterface {
        ...
        public override void ChanMsg(ChannelMessage message) {
            ...
            foreach (ClonedUser user in Clones) Commands.SendNotice(message.Connection, message.Sender.Nick, String.Format(String.Format("Name: {0}. Nickname: {1}", user.channel, user.nick)));
        ...
        public override void NickChange(NickChangeMessage message) {
            ...
            foreach (var user in Clones.Where(user => user.nick == message.Sender.Nick)) nickdata = user;
